...
const cardModes: { value: CardMode; label: string; copy: string }[] = [
  {
    value: "virtual",
    label: "Virtual card",
    copy: "A designed online invite.",
  },
  {
    value: "uploaded",
    label: "Uploaded card",
    copy: "Attach your printed card.",
  },
  {
    value: "both",
    label: "Both",
    copy: "Show the card and a digital version.",
  },
];
...
function createInitialInvite(): InvitePayload {
  return {
    ...
    hostName: "",
    message:
      "We would love for you to join us as we celebrate this special occasion.",
    date: "",
    time: "",
    venue: "",
    mapLink: "",
...
function normalizeSavedInvite(invite: SavedInvite): SavedInvite {
  ...
  return {
    ...
    url:
      ...
      (invite.publicToken || payload.publicToken || invite.id || payload._id)
...
function readSavedInvites(): SavedInvite[] {
  ...
  try {
    return (
      JSON.parse(localStorage.getItem(savedInvitesKey) || "[]") as SavedInvite[]
    ).map(normalizeSavedInvite);
  } catch {
    return [];
  }
}
...
function getDisplayNames(invite: InvitePayload) {
  ...
  if (invite.honoreeName) {
    return invite.age
      ? `${invite.honoreeName} turns ${invite.age}`
      : invite.honoreeName;
  }
...
export default function CreateInvitePage() {
  ...
  const guestStats = useMemo(() => {
    const sent = form.guestList.filter(
      (guest) => guest.status === "Sent",
    ).length;
    const opened = form.guestList.filter(
  ...
  function updateForm<K extends keyof InvitePayload>(
    key: K,
    value: InvitePayload[K],
  ) {
    ...
    try {
      ...
      nextInvites.forEach((invite) => {
        localStorage.setItem(
          `joinus:invite:${invite.id}`,
          JSON.stringify(invite.payload),
        );
      });
    } catch {
      setStatusMessage(
        "The invite was created, but local storage is full for large uploads.",
      );
    }
  }
  ...
  async function handleGalleryUpload(event: ChangeEvent<HTMLInputElement>) {
    ...
    const assets = await Promise.all(
      files.map((file) => fileToAsset(file, "gallery")),
    );
  ...
  function updateLocalGuestStatus(
    inviteId: string,
    guestId: string,
    status: GuestRecipient["status"],
  ) {
      ...
      const nextGuestList = invite.payload.guestList.map((guest) =>
        guest.id === guestId
          ? {
              ...
              status,
              lastSentAt:
                status === "Sent" ? new Date().toISOString() : guest.lastSentAt,
            }
          : guest,
      );
      ...
      return {
        ...
        payload: {
          ...
          analytics: {
            ...
            sentCount: nextGuestList.filter((guest) => guest.status === "Sent")
              .length,
          },
        },
      };
    ...
    if (createdInvite?.id === inviteId) {
      const refreshedInvite =
        updatedInvites.find((invite) => invite.id === inviteId) || null;
  ...
  async function refreshInviteStats(inviteId: string) {
    if (inviteId.startsWith("local_")) {
      setStatusMessage(
        "Local invites can be shared and previewed on this browser.",
      );
      return;
    }
  ...
  function openShare(
    ...
    if (channel === "WhatsApp") {
      ...
      window.open(
        `https://wa.me/${phone ? phone : ""}?text=${encodedMessage}`,
        "_blank",
        "noopener,noreferrer",
      );
      return;
    ...
    if (channel === "Email") {
      ...
      window.open(
        `mailto:${trimmedContact}?subject=${subject}&body=${encodedMessage}`,
        "_blank",
      );
      return;
    }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (status !== "authenticated") {
      setStatusMessage(
        "Please continue with Google before creating invitations.",
      );
      return;
    }
    ...
    const payload: InvitePayload = {
      ...
      analytics: {
        openCount: 0,
        sentCount: form.guestList.filter((guest) => guest.status === "Sent")
          .length,
        rsvpYes: 0,
        rsvpNo: 0,
      },
    ...
    try {
      ...
      id = data.id;
      payload.publicToken =
        data.publicToken || data.invite?.publicToken || data.id;
    } catch {
      localOnly = true;
      id = `local_${createId("invite")}`;
      payload.publicToken = id;
      setStatusMessage(
        "MongoDB is unavailable, so this invite was saved locally in this browser.",
      );
    }
      ...
      <header className="border-b border-stone-300 bg-[#fffaf2]/90">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-5 py-5 md:flex-row md:items-center md:justify-between">
          <div>
            <Link
              href="/"
              className="text-sm font-semibold uppercase tracking-wide text-teal-800"
            >
              JoinUs
            </Link>
          ...
          <div className="flex flex-col gap-3 md:items-end">
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              ...
              <div className="rounded-md border border-stone-300 bg-white px-4 py-2">
                <p className="text-xl font-semibold text-coral-700">
                  {guestStats.opened}
                </p>
                <p className="text-xs uppercase tracking-wide text-stone-500">
                  Opened
        ...
        <section className="mx-auto max-w-3xl px-5 py-16">
          <div className="rounded-lg border border-stone-300 bg-[#fffaf2] p-6 text-center shadow-sm md:p-10">
            ...
            <p className="mx-auto mt-4 max-w-xl leading-7 text-stone-700">
              Creators use Google to save and manage invitations. Guests can
              still open and respond to invitation links without logging in.
            </p>
            <button
              type="button"
            ...
            <p className="mt-4 text-sm text-stone-500">
              Add `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` in `.env` before
              using this button.
            </p>
          </div>
        </section>
        ...
        <div className="mx-auto max-w-7xl px-5 py-8">
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1.08fr)_minmax(360px,0.92fr)]">
            <section className="rounded-lg border border-stone-300 bg-[#fffaf2] shadow-sm">
              <div className="border-b border-stone-300 p-4">
                <div className="grid grid-cols-4 gap-2">
                  {(
                    ["details", "card", "guests", "review"] as BuilderTab[]
                  ).map((tab) => (
                    <button
                      key={tab}
                      type="button"
              ...
              <form onSubmit={handleSubmit} className="p-5">
                  ...
                  <div className="animate-rise-in space-y-6">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            updateForm("honoreeName", event.target.value)
                          }
                          className={inputClass}
                        />
                      </div>
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                        <div>
                          ...
                          <input
                            ...
                            value={form.bride}
                            onChange={(event) =>
                              updateForm("bride", event.target.value)
                            }
                            className={inputClass}
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            value={form.groom}
                            onChange={(event) =>
                              updateForm("groom", event.target.value)
                            }
                            className={inputClass}
                          />
                        </div>
                    ...
                    <div className="grid gap-4 md:grid-cols-3">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            updateForm("dressCode", event.target.value)
                          }
                          className={inputClass}
                        />
                      </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={form.mapLink}
                        onChange={(event) =>
                          updateForm("mapLink", event.target.value)
                        }
                        className={inputClass}
                      />
                    </div>
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            updateForm("rsvpContact", event.target.value)
                          }
                          className={inputClass}
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          value={form.rsvpDate}
                          onChange={(event) =>
                            updateForm("rsvpDate", event.target.value)
                          }
                          className={inputClass}
                        />
                      </div>
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            updateForm("registryLink", event.target.value)
                          }
                          className={inputClass}
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          value={form.giftNote}
                          onChange={(event) =>
                            updateForm("giftNote", event.target.value)
                          }
                          className={inputClass}
                        />
                      </div>
                    ...
                    <div>
                      ...
                      <div className="mt-2 grid grid-cols-2 gap-3 md:grid-cols-4">
                        {themes.map((theme) => (
                          ...
                          >
                            <span
                              className={`block h-8 rounded-md ${theme.swatch}`}
                            />
                            <span className="mt-2 block text-sm font-semibold text-stone-800">
                              {theme.label}
                            </span>
                          </button>
                        ))}
                ...
                {activeTab === "review" && (
                  <div className="animate-rise-in space-y-5">
                      ...
                      <div className="rounded-lg border border-stone-300 bg-white p-5">
                        ...
                        <dl className="mt-4 grid gap-3 text-sm md:grid-cols-2">
                          ...
                          <div className="rounded-md bg-stone-50 p-3">
                            ...
                            <dd className="mt-1 text-stone-950">
                              {form.gallery.length} gallery items
                            </dd>
                          </div>
                        </dl>
                      </div>
                    ...
                    {createdInvite &&
                      createdInvite.payload.guestList.length > 0 && (
                        <div className="rounded-lg border border-stone-300 bg-white p-5">
                            ...
                            {createdInvite.payload.guestList.map((guest) => (
                              ...
                              >
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    shareWithGuest(createdInvite, guest)
                                  }
                                  className="rounded-md bg-stone-950 px-4 py-2 text-sm font-semibold text-white hover:bg-teal-900"
                                >
                                  Send
                  ...
                  <div className="mt-8 flex flex-col gap-3 border-t border-stone-200 pt-5 sm:flex-row sm:justify-between">
                    <button
                      ...
                      onClick={() => {
                        ...
                        setActiveTab(
                          order[Math.max(0, order.indexOf(activeTab) - 1)],
                        );
                      }}
                      className="rounded-md border border-stone-400 px-4 py-2 text-sm font-semibold text-stone-800 hover:border-teal-700"
                    >
            ...
            <aside className="space-y-4 lg:sticky lg:top-6 lg:self-start">
              <section
                className={`rounded-lg border-2 p-5 shadow-sm ${themeClasses.frame}`}
              >
                <div className="border-b border-stone-300 pb-4">
                  <p
                    className={`text-sm font-semibold uppercase tracking-wide ${themeClasses.accent}`}
                  >
                ...
                {(form.cardMode === "uploaded" || form.cardMode === "both") &&
                  form.cardAsset && (
                    <div className="mt-5 overflow-hidden rounded-lg border border-stone-300 bg-white">
                      {form.cardAsset.kind === "image" &&
                      form.cardAsset.dataUrl ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img
                          src={form.cardAsset.dataUrl}
                          alt={form.cardAsset.name}
              ...
              {statusMessage && (
                <p className="rounded-md border border-stone-300 bg-white p-3 text-sm text-stone-700">
                  {statusMessage}
                </p>
              )}
            </aside>
          ...
          <section
            id="tracking"
            className="mt-8 rounded-lg border border-stone-300 bg-[#fffaf2] p-5 shadow-sm"
          >
            <div className="flex flex-col gap-3 border-b border-stone-300 pb-4 md:flex-row md:items-end md:justify-between">
              ...
              <p className="text-sm text-stone-600">
                {savedInvites.length} saved in this browser
              </p>
            </div>
            ...
            {savedInvites.length === 0 ? (
              ...
              <div className="mt-5 grid gap-4">
                {savedInvites.map((invite) => {
                  ...
                  const sentCount = invite.payload.guestList.filter(
                    (guest) => guest.status === "Sent",
                  ).length;
                    ...
                    >
                        ...
                        <div className="grid grid-cols-3 gap-2 text-center">
                          <div className="rounded-md bg-stone-50 px-3 py-2">
                            <p className="text-lg font-semibold text-stone-950">
                              {invite.payload.guestList.length}
                            </p>
                            <p className="text-xs uppercase tracking-wide text-stone-500">
                              Guests
                            </p>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            openShare("WhatsApp", invite.payload, invite.url)
                          }
                          className="rounded-md border border-stone-300 px-3 py-2 text-sm font-semibold text-stone-800 hover:border-teal-700"
                        >
                          WhatsApp
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            openShare("Email", invite.payload, invite.url)
                          }
                          className="rounded-md border border-stone-300 px-3 py-2 text-sm font-semibold text-stone-800 hover:border-teal-700"
                        >
                          Email
